...
public class XtendFindUsagesTest extends LightXtendTest {
  ...
  public void testFindUsagesWihtGeneratedElement_01() {
    ...
    final PsiFile generatedSource = IterableExtensions.<PsiFile>head(this.getGeneratedSources(sourceFile, _function));
  ...
  protected PsiNamedElement getNamedElementAt(final PsiFile file, final String substring) {
    ...
    {
      final int offset = file.getText().indexOf(substring);
      _xblockexpression = PsiTreeUtil.<PsiNamedElement>getParentOfType(file.findElementAt(offset), PsiNamedElement.class, false);
    }
    return _xblockexpression;
  }
  
  protected void testHighlightUsages(final PsiElement element, final String expectation) {
    TestCase.assertEquals(expectation, this.printHighlightUsages(this.getHighlightUsagesHandler(element), element));
  }
  ...
  protected String printHighlightUsages(final FindUsagesHandler findUsagesHandler, final PsiElement element) {
    String _xblockexpression = null;
    {
      ...
      final Comparator<PsiReference> _function = (PsiReference $0, PsiReference $1) -> {
        return Integer.valueOf($0.getElement().getTextOffset()).compareTo(Integer.valueOf($1.getElement().getTextOffset()));
      };
      final List<PsiReference> references = IterableExtensions.<PsiReference>sortWith(findUsagesHandler.findReferencesToHighlight(element, scope), _function);
      StringConcatenation _builder = new StringConcatenation();
  ...
  protected void testFindUsages(final PsiElement element, final String expectation) {
    TestCase.assertEquals(expectation, this.printUsages(this.getFindUsagesHandler(element)));
  }
    ...
    StringConcatenation _builder = new StringConcatenation();
    {
      for(final PsiElement element : elements) {
        ...
        final Comparator<UsageInfo> _function = (UsageInfo $0, UsageInfo $1) -> {
          return Integer.valueOf($0.getNavigationOffset()).compareTo(Integer.valueOf($1.getNavigationOffset()));
        };
        final List<UsageInfo> usages = IterableExtensions.<UsageInfo>sortWith(this.findUsages(findUsagesHandler, element), _function);
        {
  ...
  protected FindUsagesHandler getHighlightUsagesHandler(final PsiElement element) {
    return this.getFindManager().getFindUsagesManager().getFindUsagesHandler(element, true);
  }
  
  protected FindUsagesHandler getFindUsagesHandler(final PsiElement element) {
    return this.getFindManager().getFindUsagesManager().getFindUsagesHandler(element, false);
  }
  
  protected FindManagerImpl getFindManager() {
    FindManager _instance = FindManager.getInstance(this.myFixture.getProject());
