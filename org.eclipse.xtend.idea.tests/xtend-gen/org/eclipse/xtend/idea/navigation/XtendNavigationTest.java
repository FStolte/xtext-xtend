...
public class XtendNavigationTest extends LightXtendTest {
  ...
  protected <T extends Object> T testNavigateTo(final String relativePath, final String fileText, final Class<T> expectedType) {
    T _xblockexpression = null;
    {
      ...
      this.openFileInEditor(relativePath, navigationTestData.getModel());
      ...
      _xblockexpression = this.<T>assertReference(navigationTestData.getReferenceOffset(), expectedType, navigationElement);
    }
  ...
  protected <T extends Object> T assertReference(final int referenceOffset, final Class<T> expectedType, final PsiElement expectedNavigationElement) {
    T _xblockexpression = null;
    {
      ...
      if (_notEquals) {
        TestCase.assertEquals(DebugUtil.psiToString(this.getFile(), true, true), expectedNavigationElement, actualNavigationElement);
      }
  ...
  protected PsiElement findNavigationElement(final TextRange range) {
    ...
    {
      PsiElement element = this.getFile().findElementAt(range.getStartOffset());
      TestCase.assertNotNull(element);
      final PsiElement namedEObject = this.findPsiNamedEObject(element, range);
      if ((namedEObject != null)) {
        TestCase.assertEquals(namedEObject, namedEObject.getNavigationElement());
        return namedEObject;
      }
      ...
      TestCase.assertEquals(element, element.getNavigationElement());
  ...
  protected PsiElement findPsiNamedEObject(final PsiElement element, final TextRange identifierRange) {
    ...
    return this.findPsiNamedEObject(element.getParent(), identifierRange);
  }
