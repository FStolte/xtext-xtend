...
public class IdeaJavaConverterTest extends AbstractModelTestCase {
  ...
  private static class Delegate extends JavaConverterTest {
    ...
    public XtendFile file(final String code, final boolean validate) {
      return this.modelChecker.<XtendFile>checkModel(this.getFileName(code), code, validate);
    }
  ...
  protected void setUp() throws Exception {
    super.setUp();
    this.getXtextLanguage().injectMembers(this.delegate);
    this.delegate.setUp();
  }
  ...
  public void testSimpleCase() {
    ...
    final JavaConverter.ConversionResult result = this.converter.toXtend(javaCalzz.getVirtualFile().getNameWithoutExtension(), javaCalzz.getText(), this.myModule);
    TestCase.assertNotNull(result);
    TestCase.assertTrue(IterableExtensions.isEmpty(result.getProblems()));
  }
  ...
  public void testCollectJavaFilesInHandler() {
    ...
    PsiDirectory _parent = javaCalzz.getParent().getParent();
    final Iterable<PsiJavaFile> result = ConvertJavaCodeHandler.collectJavaFiles(new PsiElement[] { _parent, otherClazz });
    TestCase.assertTrue(result.iterator().hasNext());
    TestCase.assertEquals(3, IteratorExtensions.size(result.iterator()));
  }
